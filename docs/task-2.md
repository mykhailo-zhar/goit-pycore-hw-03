...
1. **Генерація та повернення результату:**
...
Припустимо, вам потрібно вибрати 6 унікальних чисел для лотерейного білета, де числа повинні бути у діапазоні від 1 до 49. Ви можете використати вашу функцію так:
...
1. Якщо параметри не відповідають заданим обмеженням, функція повертає пустий список.
1. Функція генерує вказану параметром `quantity` кількість унікальних чисел у діапазоні `[min, max]` у вигляді масиву.
1. Числа в наборі не повинні повторюватись.

## Коментарі

1. У випадку, якщо різниця між числами `max - min >= quantity - 1`, тоді доцільно повернути масив усіх елементів через `range(min, max + 1)`.   
1. В іншому випадку, функція повертає список випадкових відсортованих чисел.