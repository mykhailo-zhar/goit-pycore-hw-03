...
1. Якщо функція `normalize_phone(phone_number)`, приймає не рядок, то вона має згенерувати виключення `TypeError`
1. Результатом виконання функції має стати рядок формату `+\d{12,15}`
1. Якщо цифр менше ніж 12 чи більше ніж 15 функція має згенерувати виключення `ValueError`
1. Рядок, повернутий із функції, має починатись із `+380`
